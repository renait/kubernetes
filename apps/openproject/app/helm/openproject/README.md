...
| `persistence.nextcloud.storageClass` | PVC Storage Class for ownCloud volume     | `nil` (uses alpha storage class annotation)             |
| `persistence.nextcloud.existingClaim`| An Existing PVC name for ownCloud volume  | `nil` (uses alpha storage class annotation)             |
| `persistence.nextcloud.accessMode`   | PVC Access Mode for ownCloud volume       | `ReadWriteOnce`                                         |
| `persistence.nextcloud.size`         | PVC Storage Request for ownCloud volume   | `8Gi`                                                   |
